...
from common.api import CFBAPIConnector


async def build_current_team_database(team: str, year: str) -> pd.DataFrame:
    """
    ...
    games = await client.get_records({"team": team, "season": season})
...
def build_current_team_database(team: str, season: str) -> dict:
    ...
    for idx, game in enumerate(games):
        ...
        if game["home_points"] is None:
            ...
            DB["next_game"] = {
                "opponent": game[opponent_key + "team"],
                "date": game["start_date"].split("T")[0],
            }
            DB["last_game"] = {
                "date": prev["start_date"].split("T")[0],
                "win": True
                if prev[prev_team_key + "points"] > prev[prev_opponent_key + "points"]
                else False,
